...
CREATE SCHEMA IF NOT EXISTS ck_demo_staging;

	DROP TABLE IF EXISTS ck_demo_staging.daily_dogs;
	DROP TABLE IF EXISTS ck_demo_staging.dogs_schedule_changes;
	DROP TABLE IF EXISTS ck_demo_staging.clients_schedule;
	DROP TABLE IF EXISTS ck_demo_staging.dogs;
	DROP TABLE IF EXISTS ck_demo_staging.clients;
	DROP TABLE IF EXISTS ck_demo_staging.routes;
	DROP TABLE IF EXISTS ck_demo_staging.employees_schedule;
	DROP TABLE IF EXISTS ck_demo_staging.employees_schedule_changes;
	DROP TABLE IF EXISTS ck_demo_staging.admin_notes;
	DROP TABLE IF EXISTS ck_demo_staging."user";
	DROP TABLE IF EXISTS ck_demo_staging.employees;
	DROP TABLE IF EXISTS ck_demo_staging.services;

CREATE TABLE ck_demo_staging.employees (
...
insert into ck_demo_staging.employees 
...
CREATE TABLE ck_demo_staging."user" (
	"id" SERIAL PRIMARY KEY,
	"emp_id" INT REFERENCES ck_demo_staging.employees(id) ON DELETE CASCADE,
...
--** USERNAME: 'admin'
--** PASSWORD: 'admin'
INSERT INTO ck_demo_staging."user"
...
CREATE TABLE ck_demo_staging.employees_schedule (
	"id" SERIAL PRIMARY KEY,
	"emp_id" INT NOT NULL REFERENCES ck_demo_staging.employees(id) ON DELETE CASCADE ,
	"week" INT NOT NULL,
...
insert into ck_demo_staging.employees_schedule
...
-- ADDED EMP SCHEDULE CHANGES TABLE ck_demo_staging.(Yani)
CREATE TABLE ck_demo_staging.employees_schedule_changes (
	"id" SERIAL PRIMARY KEY,
...
CREATE TABLE ck_demo_staging.routes (
	"id" SERIAL PRIMARY KEY,
	"name" VARCHAR(150)
	);

INSERT INTO ck_demo_staging.routes
	("name")
VALUES
	('Tangletown'),
	('Emerson'),
...
CREATE TABLE ck_demo_staging.services (
	"id" SERIAL PRIMARY KEY,
	"name" VARCHAR (150),
	"price" INT
);

INSERT INTO ck_demo_staging.services
	("name", "price")
VALUES
...
CREATE TABLE ck_demo_staging.clients (
	"id" SERIAL PRIMARY KEY,
	"first_name" VARCHAR(150) NOT NULL,
	"last_name" VARCHAR(150),
	"street" VARCHAR(150),
...
--	"service_id" INT NOT NULL REFERENCES,
	"route_id" INT NOT NULL REFERENCES ck_demo_staging.routes(id),
	"phone" VARCHAR(13),
	"email" VARCHAR(150) NOT NULL,
	"notes" VARCHAR,
	"date" DATE DEFAULT CURRENT_DATE,
...
insert into ck_demo_staging.clients 
...
CREATE TABLE ck_demo_staging.dogs (
	"id" SERIAL PRIMARY KEY,
	"client_id" INT NOT NULL REFERENCES ck_demo_staging.clients(id) ON DELETE CASCADE,
	"name" VARCHAR(150),
	"image" VARCHAR,
...
insert into ck_demo_staging.dogs 
...
-- ** Changed table ck_demo_staging.name to client_schedule since the days will be set for the client and changes will be made to individual dogs.
-- ** Changed the weekday column titles from m-f to (1-5) to make using MUI calendar data easier.
CREATE TABLE ck_demo_staging.clients_schedule (
	"id" SERIAL PRIMARY KEY,
	"client_id" INT NOT NULL REFERENCES ck_demo_staging.clients(id) ON DELETE CASCADE,
	"1" BOOLEAN DEFAULT FALSE,
	"2" BOOLEAN DEFAULT FALSE,
...
insert into ck_demo_staging.clients_schedule
	("client_id", "1", "2", "3", "4", "5") 
values
...
CREATE TABLE ck_demo_staging.dogs_schedule_changes (
	"id" SERIAL PRIMARY KEY,
	"dog_id" INT NOT NULL REFERENCES ck_demo_staging.dogs(id) ON DELETE CASCADE,
	"client_id" INT NOT NULL REFERENCES ck_demo_staging.clients(id) ON DELETE CASCADE,
	"date_to_change" DATE NOT NULL,
	"is_scheduled" BOOLEAN,
	"date" DATE DEFAULT CURRENT_DATE,
...
CREATE TABLE ck_demo_staging.daily_dogs (
	"id" SERIAL PRIMARY KEY,
	"name" VARCHAR(150) NOT NULL,
	"date" DATE DEFAULT CURRENT_DATE,
	"dog_id" INT NOT NULL REFERENCES ck_demo_staging.dogs(id),
	"route_id" INT NOT NULL REFERENCES ck_demo_staging.routes(id),
	"client_id" INT NOT NULL REFERENCES ck_demo_staging.clients(id),
...
insert into ck_demo_staging.daily_dogs
...
CREATE TABLE ck_demo_staging.admin_notes (
	"id" SERIAL PRIMARY KEY,
	"user_id" INT NOT NULL REFERENCES ck_demo_staging."user"(id) ON DELETE CASCADE,
	"notes" VARCHAR
	);

INSERT INTO ck_demo_staging.admin_notes
	("user_id", "notes")
VALUES
