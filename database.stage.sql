CREATE SCHEMA IF NOT EXISTS ck_stage;
...
CREATE TABLE ck_stage."user" (
	"id" SERIAL PRIMARY KEY,
	"emp_id" INT REFERENCES ck_stage.employees(id) ON DELETE CASCADE,
...
CREATE TABLE ck_stage.employees_schedule (
	"id" SERIAL PRIMARY KEY,
...
-- ADDED EMP SCHEDULE CHANGES TABLE ck_stage.(Yani)
CREATE TABLE ck_stage.employees_schedule_changes (
	"id" SERIAL PRIMARY KEY,
	"emp_id" INT NOT NULL,   --REFERENCES ck_stage.clients(id) ON DELETE CASCADE--
...
CREATE TABLE ck_stage.routes (
	"id" SERIAL PRIMARY KEY,
	"name" VARCHAR(150)
	);

INSERT INTO ck_stage.routes
	("name")
VALUES
	('Tangletown'),
	('Emerson'),
...
CREATE TABLE ck_stage.services (
	"id" SERIAL PRIMARY KEY,
	"qb_id" INT NULL,
	"name" VARCHAR (150),
	"price" INT
...
--	"service_id" INT NOT NULL REFERENCES,
	"route_id" INT REFERENCES ck_stage.routes(id),
	"phone" VARCHAR(150),
	"mobile" VARCHAR(150),
	"email" VARCHAR(150) NOT NULL,
	"notes" VARCHAR,
...
-- ** Changed table ck_stage.name to client_schedule since the days will be set for the client and changes will be made to individual dogs.
-- ** Changed the weekday column titles from m-f to (1-5) to make using MUI calendar data easier.
CREATE TABLE ck_stage.clients_schedule (
	"id" SERIAL PRIMARY KEY,
	"qb_id" INT NOT NULL,
	"client_id" INT NOT NULL REFERENCES ck_stage.clients(id) ON DELETE CASCADE,
	"1" BOOLEAN DEFAULT FALSE,
...
CREATE TABLE ck_stage.dogs_schedule_changes (
	"id" SERIAL PRIMARY KEY,
	"dog_id" INT NOT NULL REFERENCES ck_stage.dogs(id) ON DELETE CASCADE,
	"client_id" INT NOT NULL REFERENCES ck_stage.clients(id) ON DELETE CASCADE,
	"date_to_change" DATE NOT NULL,
	"is_scheduled" BOOLEAN,
	"date" DATE DEFAULT CURRENT_DATE,
...
CREATE TABLE ck_stage.daily_dogs (
	"id" SERIAL PRIMARY KEY,
	"name" VARCHAR(150) NOT NULL,
	"date" DATE DEFAULT CURRENT_DATE,
	"dog_id" INT NOT NULL REFERENCES ck_stage.dogs(id),
	"route_id" INT NOT NULL REFERENCES ck_stage.routes(id),
	"client_id" INT NOT NULL REFERENCES ck_stage.clients(id) ON DELETE CASCADE,
